...
Path Graph::getShortestPath(Node, Node) {
...
void Graph::buildAllShortestPath() {
	...
	for (int i = 0; i < edgeVector.size(); ++i) {
		...
		int x = std::distance(nodeVector.begin(), std::find(nodeVector.begin(), nodeVector.end(), edge.inNode));
		int y = std::distance(nodeVector.begin(), std::find(nodeVector.begin(), nodeVector.end(), edge.outNode));
		edges[x].push_back(y);
	}

	static std::vector<std::vector<int> > dist(nodeVector.size(), std::vector<int>(nodeVector.size()));
	static std::vector<std::vector<int> > pre_node(nodeVector.size(), std::vector<int>(nodeVector.size()));
	...
	for (int from = 0; from < nodeVector.size(); ++from) {
		...
		while (!q.empty()) {
			...
			if (dist[from][x.index] >= x.dis) {
				...
				if (x.index != from && nodeVector[x.index].type == TOLLSTATION) continue; // 收费站不能再往下转移
